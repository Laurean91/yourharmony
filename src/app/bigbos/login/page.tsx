...
export default function LoginPage() {
  ...
  async function handlePassword(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Проверяем активирован ли TOTP
    const totpRes = await fetch('/api/auth/totp/status').then(r => r.json()).catch(() => ({ enabled: false }))
  ...
  return (
    <div
      ...
      style={{ background: 'linear-gradient(135deg, #1c1045 0%, #130b35 50%, #0f0828 100%)' }}
    >
      ...
      <div className="hidden lg:flex flex-col justify-between p-12 w-[420px] shrink-0 relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 relative z-10">
          ...
          <div>
            <p className="text-xl font-extrabold leading-tight"
              style={{ background: 'linear-gradient(90deg, #c4b5fd 0%, #fb923c 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              Гармония
            </p>
        ...
        <div className="relative z-10">
          <p className="text-3xl font-extrabold leading-snug mb-4" style={{ color: 'rgba(255,255,255,0.9)' }}>
            ...
            <span style={{ background: 'linear-gradient(90deg, #c4b5fd 0%, #fb923c 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              клуба Гармония
            </span>
          </p>
        ...
        <div className="w-full max-w-sm rounded-2xl p-8"
          ...
          {step === 'password' ? (
            <>
              ...
              <form onSubmit={handlePassword} className="space-y-4">
                ...
                <button type="submit" disabled={loading}
                  ...
                  style={{
                    background: loading ? 'linear-gradient(90deg, #9ca3af 0%, #9ca3af 100%)' : 'linear-gradient(90deg, #7c3aed 0%, #6d28d9 100%)',
            ...
            <>
              ...
              <h1 className="text-2xl font-extrabold text-gray-900 mb-1 text-center">Двухфакторная<br />аутентификация</h1>
              <p className="text-sm text-gray-400 mb-7 text-center">
                Введите 6-значный код<br />из приложения Google Authenticator
              </p>
              ...
              <form onSubmit={handleTotp} className="space-y-4">
                <div>
                  <label htmlFor="code" className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide text-center">
                    Код подтверждения
                  </label>
                  <input
                    ref={codeRef}
                    ...
                    placeholder="000000"
                    className="w-full rounded-xl px-4 py-3 text-center text-2xl font-bold tracking-[0.4em] text-gray-800 outline-none"
                    style={{ border: '1.5px solid #e5e7eb', background: '#fafafa', letterSpacing: '0.4em' }}
                    onFocus={e => { e.currentTarget.style.borderColor = '#7c3aed'; e.currentTarget.style.boxShadow = '0 0 0 3px rgba(124,58,237,0.12)'; e.currentTarget.style.background = '#fff' }}
                    onBlur={e  => { e.currentTarget.style.borderColor = '#e5e7eb'; e.currentTarget.style.boxShadow = 'none'; e.currentTarget.style.background = '#fafafa' }}
                  />
                </div>
                ...
                <button type="submit" disabled={loading || code.length < 6}
                  className="w-full flex items-center justify-center gap-2.5 py-3 rounded-xl text-sm font-bold text-white transition-all mt-2"
                  style={{
                    background: loading || code.length < 6 ? '#9ca3af' : 'linear-gradient(90deg, #7c3aed 0%, #6d28d9 100%)',
                    boxShadow: loading || code.length < 6 ? 'none' : '0 4px 20px rgba(124,58,237,0.35)',
                ...
                <button type="button" onClick={() => { setStep('password'); setError(''); setCode('') }}
