...
import { Calendar, Star, CheckSquare, ChevronRight } from 'lucide-react'
  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-400 mt-1">Личный кабинет клуба «Гармония»</p>
      </div>
        ...
        {parent.students.map(({ student }) => {
          ...
          return (
            <div key={student.id} className="rounded-2xl overflow-hidden"
              style={{ background: '#fff', boxShadow: '0 4px 24px rgba(5,150,105,0.08)', border: '1px solid #d1fae5' }}>
              ...
              <div className="grid grid-cols-3 divide-x divide-gray-100 border-b border-gray-100">
                <div className="px-2 py-3 text-center">
                  <p className="text-[10px] text-gray-400 mb-0.5 leading-tight">Посещ.</p>
                  <p className="text-base font-black" style={{ color: '#059669' }}>
                    {total > 0 ? `${Math.round((attended / total) * 100)}%` : '—'}
                  </p>
                </div>
                <div className="px-2 py-3 text-center">
                  <p className="text-[10px] text-gray-400 mb-0.5 leading-tight">Уроков</p>
                  <p className="text-base font-black text-gray-800">{total}</p>
                </div>
                <div className="px-2 py-3 text-center">
                  <p className="text-[10px] text-gray-400 mb-0.5 leading-tight">Оценка</p>
                  <p className="text-base font-black" style={{ color: lastGrade ? '#3b82f6' : '#9ca3af' }}>
                    {lastGrade?.grade ?? '—'}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 divide-x divide-gray-100">
                {[
                  ...
                  <Link key={href} href={href}
                    className="flex flex-col items-center gap-1.5 py-4 text-xs font-medium text-gray-500 hover:text-emerald-600 hover:bg-emerald-50 transition-all min-h-[44px]">
                    <Icon size={18} />
                    {label}
                  </Link>
                ))}
