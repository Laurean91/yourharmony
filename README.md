...
### UX-улучшения форм, очистка кода (2026-03-23)

- **Форма записи**: снято ограничение возраста (было `max="18"`), теперь без ограничений
- **Согласие на обработку данных**: галочка заменена на красивый кастомный чекбокс (анимация, фиолетовый акцент)
- **Блог**: удалён блок «Автор» в конце каждой статьи
- **Очистка**: удалены неиспользуемые компоненты `PostCard.tsx`, `WelcomeGate.tsx`, `WelcomeOverlay.tsx`
- **Редактирование и перенос занятий** в расписании через Server Actions `updateLesson` и `moveLessonDate`
