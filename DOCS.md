...
### SVG-фавиконка с новым логотипом (2026-03-21)
- `src/app/icon.svg` — новая фавиконка: два речевых пузыря (фиолетовый + оранжевый), три звёздочки, улыбка
- Next.js App Router автоматически подхватывает `icon.svg` как favicon для современных браузеров
- Старые браузеры продолжают использовать `favicon.ico`
- SVG масштабируется без потери качества на Retina и любых DPI
...
### Обновление текстов и SEO-данных (2026-03-22)

- **Возраст**: все тексты обновлены с «от 6 лет» на **«от 4 лет»** (Hero, Features, Formats, FAQ, schema)
- **Расписание**: групповые занятия — **суббота, 12:00** (ранее «2 раза в неделю»); онлайн — **по согласованию с преподавателем**
- **Убрано ложное обещание «бесплатного пробного занятия»** из всех компонентов: Hero-кнопка, `BookingButton`, CTA-секция, HowItWorks, FAQ, blog/page CTA
- **FAQ** — заменён вопрос о «бесплатном пробном» на «Когда проходят групповые занятия?» с адресом
- **Форматы**: секция «Индивидуальные занятия» переименована в «Онлайн-занятия», описание уточнено
- **Schema (LocalBusiness)**: `foundingDate` исправлен с `2020` на `2023`; `openingHoursSpecification` обновлён — только суббота 12:00–18:00
- **Schema (Course)**: расписание `byDay: ['Saturday'], startTime: '12:00'`; онлайн-инстанс с описанием
- **Мета-описание** (`layout.tsx`): включает расписание, адрес, убрано «бесплатное пробное»
- **Очистка репозитория**: удалены мусорные файлы (скриншоты аудита, прототипы редизайна, playwright-логи); `.gitignore` дополнен
