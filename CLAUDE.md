...
**Индексы:** `Booking(createdAt, status)`, `Post(isPublished, categoryId, createdAt)`, `Lesson(date)`, `LessonStudent(lessonId, studentId)`
...
- **Дашборд** — приветствие, карточки быстрых ссылок
...
- Мобильная оптимизация: bottom nav, touch targets ≥44px, safe-area-inset
